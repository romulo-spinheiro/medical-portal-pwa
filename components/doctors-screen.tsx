...
export function DoctorsScreen() {
  ...
  // Even if the source data has duplicates, we group by doctor ID.
  const uniqueDoctors = useMemo(() => {
    const grouped = doctors.reduce((acc, doctor) => {
      if (!acc[doctor.id]) {
        acc[doctor.id] = {
          ...doctor,
          doctorSchedules: schedules.filter(s => s.doctor_id === doctor.id)
        }
      }
      return acc
    }, {} as Record<string, Doctor & { doctorSchedules: Schedule[] }>)
  ...
  const filteredDoctors = useMemo(() => {
    return uniqueDoctors.filter((doctor) => {
      ...
      const neighborhoodMatch =
        ...
        doctor.doctorSchedules.some((s) => s.neighborhood_name === selectedNeighborhood)

      return searchMatch && specialtyMatch && neighborhoodMatch
    })
  }, [uniqueDoctors, searchQuery, selectedSpecialty, selectedNeighborhood])

  // Specialties & Neighborhoods for Filters
  const specialties = useMemo(() => {
    const list = dbSpecialties.length > 0 ? dbSpecialties.map(s => s.name) : Array.from(new Set(doctors.map(d => d.specialty_name).filter(Boolean)))
    return ["Todas", ...list.sort()]
  }, [dbSpecialties, doctors])

  const neighborhoods = useMemo(() => {
    const list = dbNeighborhoods.length > 0 ? dbNeighborhoods.map(n => n.name) : Array.from(new Set(schedules.map(s => s.neighborhood_name).filter(Boolean)))
  ...
  return (
    <div className="min-h-screen pb-24">
      ...
      <div className="sticky top-0 z-40 border-b border-white/40 bg-white/30 px-4 pb-4 pt-6 backdrop-blur-xl">
        <h1 className="mb-4 text-xl font-medium text-gray-800 tracking-tight">Médicos</h1>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">Filtrar Resultado</span>
          </div>
      ...
      <div className="space-y-4 px-4 py-6">
        <p className="px-1 text-[10px] font-medium uppercase tracking-widest text-gray-400">
          {filteredDoctors.length} {filteredDoctors.length === 1 ? "Médico listado" : "Médicos listados"}
        </p>

        {filteredDoctors.length === 0 ? (
          <div className="rounded-3xl border border-gray-100 bg-white/40 p-10 text-center backdrop-blur-sm">
            <p className="text-gray-400 text-sm font-medium">Nenhum médico encontrado.</p>
          </div>
        ) : (
          filteredDoctors.map((doctor) => (
            <button
              ...
              className="w-full rounded-2xl border border-white/80 bg-white/60 p-5 text-left shadow-lg shadow-gray-200/20 backdrop-blur-xl transition-all hover:bg-white/80 active:scale-[0.98]"
            >
              ...
              <div className="mt-4 pt-4 border-t border-gray-100/50 flex items-center justify-between text-[10px] font-medium text-gray-500 uppercase tracking-widest">
                ...
                <span className="text-[#22c55e] font-medium italic">Detalhes →</span>
              </div>
            </button>
          ))
        )}
