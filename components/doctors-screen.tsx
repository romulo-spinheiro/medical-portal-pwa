...
export function DoctorsScreen() {
  const { doctors: rawDoctors, schedules, specialties: dbSpecialties, neighborhoods: dbNeighborhoods } = useApp()
  ...
  // 1. DEDUPLICAÇÃO VISUAL (Filtro Final): Garante que IDs repetidos não gerem múltiplos cards
  const uniqueDoctors = useMemo(() => {
    return rawDoctors.filter((doc, index, self) =>
  ...
  }, [uniqueDoctors, searchQuery, selectedSpecialty, selectedNeighborhood, schedules])

  // Specialties & Neighborhoods for Filters
  const specialties = useMemo(() => {
    const list = dbSpecialties.length > 0
      ? dbSpecialties.map(s => s.name)
      : Array.from(new Set(rawDoctors.map(d => d.specialty_name).filter(Boolean)))
    return ["Todas", ...list.sort()]
  }, [dbSpecialties, rawDoctors])
    ...
    <div className="min-h-screen pb-24">
      ...
      <div className="sticky top-0 z-40 border-b border-white/40 bg-white/30 px-4 pb-4 pt-6 backdrop-blur-xl">
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Buscar médico, CRM ou especialidade..."
            value={searchQuery}
      ...
      <div className="space-y-4 px-4 py-6">
        ...
        {filteredDoctors.length === 0 ? (
          <div className="rounded-3xl border border-gray-100 bg-white/40 p-10 text-center">
            <p className="text-gray-400 text-sm font-medium">Nenhum médico com esses critérios.</p>
          </div>
        ) : (
