...
export function HomeScreen() {
  ...
  // Explicitly set today as default if not selected
  useEffect(() => {
    if (!selectedDay) {
      const today = new Date().toLocaleDateString('pt-BR', { weekday: 'long' })
      const capitalizedToday = today.charAt(0).toUpperCase() + today.slice(1)
      // Check if it's one of our DAYS_OF_WEEK
      if (DAYS_OF_WEEK.includes(capitalizedToday)) {
        setSelectedDay(capitalizedToday)
      } else {
        setSelectedDay("Segunda") // Fallback
      }
    }
  }, [])
  ...
  const neighborhoods = useMemo(() => {
    const list = dbNeighborhoods.length > 0
      ...
      : Array.from(new Set(schedules.map(s => s.neighborhood_name).filter((n): n is string => Boolean(n))))
  ...
  // CORE LOGIC: GROUPING SCHEDULES BY DOCTOR TO AVOID DUPLICATES
  const groupedSchedules = useMemo(() => {
    // 1. First filter the raw rows by day/neighborhood
    ...
    filtered.forEach(item => {
      ...
      if (!groups.has(item.doctor_id)) {
        ...
        groups.get(item.doctor_id)!.items.push(item)
      }
    })
    ...
    <div className="min-h-screen pb-24">
      ...
      <div className="sticky top-0 z-40 border-b border-white/40 bg-white/30 px-4 pb-4 pt-6 backdrop-blur-xl">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={handleAvatarClick} disabled={isUploading} className="group relative h-11 w-11 shrink-0 rounded-full bg-gradient-to-br from-[#22c55e] to-[#16a34a] shadow-md border-2 border-white overflow-hidden transition-transform active:scale-90">
              {avatarUrl ? <img src={avatarUrl} className="h-full w-full object-cover" alt="Profile" /> : <span className="text-white font-medium">{userName ? userName[0].toUpperCase() : "?"}</span>}
          ...
          <button onClick={handleRefresh} disabled={isRefreshing} className="h-9 w-9 flex items-center justify-center rounded-full bg-white/60 text-gray-400 border border-white/80 transition-all active:scale-95 hover:text-[#22c55e]">
      ...
      <div className="space-y-4 px-4 py-6">
        ...
        <div className="space-y-2">
          <p className="px-1 text-[10px] font-medium uppercase tracking-widest text-gray-400">Filtrar Dia</p>
        ...
        <div className="space-y-2">
          <p className="px-1 text-[10px] font-medium uppercase tracking-widest text-gray-400">Filtrar Bairro</p>
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {neighborhoods.map(n => (
      ...
      <div className="space-y-4 px-4">
        ...
        {groupedSchedules.length === 0 ? (
          <div className="rounded-3xl border border-gray-100 bg-white/40 p-10 text-center backdrop-blur-xl"><p className="text-gray-400 font-medium text-sm">Sem atendimentos para os filtros selecionados.</p></div>
        ) : (
          groupedSchedules.map(({ doctor, items }) => (
            <button
              key={doctor.id}
              onClick={() => { setSelectedDoctor(doctor); setIsModalOpen(true); }}
              className="w-full rounded-3xl border border-white/80 bg-white/60 p-5 shadow-lg shadow-gray-200/20 backdrop-blur-xl text-left transition-all hover:bg-white/80 active:scale-[0.99]"
            >
              <div className="flex gap-4">
                ...
                <div className="flex-1 space-y-0.5">
                  ...
                  {doctor.phone && <div className="flex items-center gap-1 text-[10px] font-medium text-gray-400 uppercase tracking-widest leading-none mt-1"><Phone className="h-2.5 w-2.5" /><span>{doctor.phone}</span></div>}
                </div>
              ...
              <div className="mt-5 space-y-2">
                {items.map((s, idx) => (
                  <div key={idx} className="flex items-center justify-between rounded-2xl bg-white/50 px-4 py-2.5 border border-white/80">
                    <div className="flex items-center gap-2 text-xs font-medium text-gray-700"><Clock className="h-3.5 w-3.5 text-[#22c55e]" /><span>{s.start_time.slice(0, 5)} — {s.end_time.slice(0, 5)}</span></div>
                    <div className="flex items-center gap-1.5 text-[10px] font-medium text-gray-400 uppercase tracking-widest"><MapPin className="h-3 w-3" /><span>{s.place_name}</span></div>
                  </div>
                ))}
              </div>
            </button>
